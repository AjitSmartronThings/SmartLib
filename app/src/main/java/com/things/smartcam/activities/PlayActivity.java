...
import com.things.smartcam.Fragment.PlayFragment;
...
public class PlayActivity extends AppCompatActivity {
    ...
    private View renderHolder;
    private View liveVideoBar;
    private TextView streamBps;
    private ImageButton liveVideoBarRecord;
    private ImageButton liveVideoBarTakePicture;
    private ImageButton liveVideoBarEnableAudio;
    ...
    private final Runnable mTimerRunnable = new Runnable() {
        ...
        public void run() {
            ...
            streamBps.setText((length - mLastReceivedLength) / 1024 + "Kbps");
    ...
    private Runnable mResetRecordStateRunnable = new Runnable() {
        @Override
        public void run() {
            ImageView mPlayAudio = liveVideoBarRecord;
            ...
            case MY_PERMISSIONS_REQUEST_WRITE_EXTERNAL_STORAGE + 1: {
                ...
                if (grantResults.length > 0
                    ...
                    if (requestCode == MY_PERMISSIONS_REQUEST_WRITE_EXTERNAL_STORAGE) {
                        onTakePicture(liveVideoBarTakePicture);
                    } else {
                        onRecordOrStop(liveVideoBarRecord);
                    }
...
//        startActivity(intent);
    ...
    public void onAddWindow() {
        new AlertDialog.Builder(PlayActivity.this).setTitle("New play window").setItems(new CharSequence[]{"Select historical play history","Manually enter the video source"}, new DialogInterface.OnClickListener() {
            @Override
    ...
    public void onRemoveVideoFragment(int id) {
        ...
        playerContainer.removeView(playerContainer.findViewById(id));
    }
   ...
   /* public void onRemoveOptionMenu(VideoWindowOptionMenuFragment optionFragment) {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //url = "rtsp://192.168.0.3:554/live/av0?user=admin&passwd=admin";
        if (TextUtils.isEmpty(url)) {
            finish();
            return;
        }
        ...
        renderHolder = (View) findViewById(R.id.render_holder);
        playerContainer = (LinearLayout) findViewById(R.id.player_container);
        liveVideoBar = (View) findViewById(R.id.live_video_bar);
        liveVideoBarEnableAudio = (ImageButton) liveVideoBar.findViewById(R.id.live_video_bar_enable_audio);
        liveVideoBarTakePicture = (ImageButton) liveVideoBar.findViewById(R.id.live_video_bar_take_picture);
        liveVideoBarRecord = (ImageButton) liveVideoBar.findViewById(R.id.live_video_bar_record);
        streamBps = (TextView) liveVideoBar.findViewById(R.id.stream_bps);

        liveVideoBarEnableAudio.setEnabled(false);
        liveVideoBarTakePicture.setEnabled(false);
        liveVideoBarRecord.setEnabled(false);
        ...
        if (isLandscape()) {
            ...
            renderHolder.getLayoutParams().height = ViewGroup.LayoutParams.MATCH_PARENT;
    ...
    private void onVideoDisplayed() {
       liveVideoBarTakePicture.setEnabled(true);
        liveVideoBarRecord.setEnabled(true);
    }

    private void onPlayStart() {
        boolean enable = mRenderFragment.isAudioEnable();
        liveVideoBarEnableAudio.setImageState(enable ? new int[]{android.R.attr.state_pressed} : new int[]{}, true);
        liveVideoBarEnableAudio.setEnabled(true);
